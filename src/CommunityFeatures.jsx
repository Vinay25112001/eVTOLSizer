...
import { makeDesignRecord, recordToRow } from "./lib/designfile.js";
...
/* ══════════════════════════════════════════════════════
   SESSION / DESIGN API
   ══════════════════════════════════════════════════════ */
export async function publishDesign(userId, displayName, params, results, customAirfoil = null) {
  /* Stored as a design record (lib/designfile.js): every input, the engine
     stamp and the outputs, so whoever opens the link is told whether their
     engine still agrees. The headline numbers stay for the banner. */
  const record = makeDesignRecord({ params, customAirfoil, name: displayName });
  const row = recordToRow(record, { MTOW:results.MTOW, Etot:results.Etot, Phov:results.Phov,
...
/* ══════════════════════════════════════════════════════
   SHARE BUTTON
   ══════════════════════════════════════════════════════ */
export function ShareDesignButton({ user, params, results, customAirfoil, C }) {
  ...
  const doShare = async () => {
    ...
    try {
      ...
      setShareId(await publishDesign(user.id, name, params, results, customAirfoil));
      setPhase("done");
      ...
      {inSession && (
        <>
          ...
          <div style={{background:C.panel,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 14px"}}>
            ...
            {log.length===0
              ? <div style={{fontSize:10,color:C.subtle,fontFamily:"'DM Mono',monospace"}}>No activity yet…</div>
              : (
                <div style={{display:"flex",flexDirection:"column",gap:4,maxHeight:160,overflowY:"auto"}}>
                  {log.map(a => (
                    <div key={a.id} style={{display:"flex",gap:10,alignItems:"baseline"}}>
                      <span style={{fontSize:8,color:C.subtle,fontFamily:"'DM Mono',monospace",flexShrink:0}}>{a.t}</span>
...
export function PublicDesignBanner({ shareId, onLoad, C }) {
  ...
  return (
    <div style={{padding:"12px 18px",background:`${C.teal}11`,border:`1px solid ${C.teal}44`,borderRadius:8,marginBottom:10,display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
      ...
      <button onClick={()=>{ onLoad(design); setGone(true); }} type="button"
